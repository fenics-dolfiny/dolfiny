...
TAOConstraintsJacobianFunction = Callable[[PETSc.TAO, PETSc.Vec, PETSc.Mat, PETSc.Mat], None]
...
def link_state(u: Sequence[dolfinx.fem.Function]):
    def _decorator[T](_to_wrap: T) -> T:
        def _wrapped_callback(tao: PETSc.TAO, x: PETSc.Vec, *args):  # type: ignore
    ...
    def wrap_form_constraints(
        ...
        def _Jg_callback(tao, x, J, P) -> None:
            ...
            if Jg[0][0].rank == 2:
                ...
                for k, _Jg in enumerate(Jg[0]):  # type: ignore
                    match _Jg.rank:
                        case 0:
